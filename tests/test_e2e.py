...
from mirrormind.cli import main
...
def test_init_import_status_roundtrip(tmp_path, capsys):
    ...
    assert (vault / "mirrormind.toml").exists()
    ...
    assert "MirrorMind" in (vault / "index.md").read_text(encoding="utf-8")
    assert len(list(vault.glob(".mirrormind/chunks/*.jsonl"))) == 3
...
def test_git_autocommit(tmp_path, capsys):
    ...
    assert "mirrormind: sync 2 new" in log
